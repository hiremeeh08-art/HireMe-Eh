...
function Home({ setTab }) {
  ...
  return (
    <div>
      ...
      <div style={G.card}>
        <div style={{ fontWeight: 800, fontSize: "1.15rem", marginBottom: "0.35rem" }}>Three Ways to Use HireMe.eh</div>
...
function Jobs({ userPlan, setTab }) {
  ...
  return (
    <div>
      ...
      {loading && <div style={{ ...G.card, textAlign: "center", padding: "3rem" }}><div style={G.spinner} /><div style={{ marginTop: "1.1rem", color: C.gray, fontSize: "0.88rem" }}>Scanning Job Bank Canada, Indeed, LinkedIn & more...</div></div>}
      {results && (
        <div style={G.card}>
          <div style={{ fontWeight: 800, fontSize: "1.05rem", marginBottom: "0.2rem" }}>{results.length} Fresh GTA Postings</div>
          <div style={{ color: C.gray, fontSize: "0.78rem", marginBottom: "1.25rem" }}>Live results from Job Bank Canada + Indeed + LinkedIn</div>
          {results.map((job, i) => (
    ...
    <div style={G.app}>
      ...
      <header style={G.header}>
        <div style={{display:"flex",alignItems:"center",gap:"0.55rem",flexShrink:0}}>
          ...
          <div><div style={G.logoText}>HireMe.eh</div><div style={G.logoSub}>The Job Search App Ontario Was Waiting For</div></div>
        </div>
